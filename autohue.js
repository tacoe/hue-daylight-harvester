...
var LIGHTS = [ 1, 2, 3, 4 ];        // 'ardcode me lites, matey
var LIGHTFACTORS = [ 1, 1, 1, 1 ]; // use values < 1 to scale down global max per light
...
var STATE_IN = null;
var STATE_OUT = null;
var STATE_RESUME = { "bri": 42, "hue": 42, "on": true };
...
function getHueBrightnessAndCT() {
  ...
  request(url, function(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (!json || !json.state || !json.state.hasOwnProperty("bri")) {
        ...
        STATE_OUT = json.state;
        log("Setting STATE_OUT: " + JSON.stringify(json.state));
        if (STATE_IN != null) {
            var in_out_equal = STATE_OUT.bri == STATE_IN.bri && STATE_OUT.ct == STATE_IN.ct && STATE_OUT.on == STATE_IN.on;
            var resume_out_equal = STATE_OUT.bri == STATE_RESUME.bri && STATE_OUT.hue == STATE_RESUME.hue && STATE_OUT.on == STATE_RESUME.on;
            if (resume_out_equal) {
                log("Resuming AutoHue...")
                MODE = "ON";
            } else if (!in_out_equal) {
                log("Pauzing AutoHue...")
                MODE = "OFF";
            }
        }
...
function setHueLightBrightnessAndCT(light, bri, ct) {
  ...
  if (light == MEASURELIGHT) {
      STATE_IN = state;
      log("Setting STATE_IN: " + JSON.stringify(state));
  }
